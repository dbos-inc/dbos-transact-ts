...
async function raceFunction() {
  ...
  await dataSource.client.raw('INSERT INTO race_side_effects(tag) VALUES (?)', [`run-${raceState.callCount}`]);
...
const sysConfig = { ...config.connection, database: 'knex_ds_test_dbos_sys' };
...
async function staleFunction() {
  await dataSource.client.raw("INSERT INTO race_side_effects(tag) VALUES ('stale')");
...
describe('KnexDataSource migrations', () => {
  const userDB = new Pool(config.connection);
  ...
  test('verify-only initialization requires the schema to be migrated out of band', async () => {
    ...
    const admin = new Client({ ...config.connection, database: 'postgres' });
    await admin.connect();
    try {
      await ensureDB(admin, config.connection.database);
    } finally {
      await admin.end();
    }
    await userDB.query(`CREATE ROLE "${role}"`);
    try {
      new KnexDataSource(`verify-only-${suffix}`, config, schemaName, { runMigrations: false });
      DBOS.setConfig({ name: 'knex-ds-test' });
      ...
      await KnexDataSource.initializeDBOSSchema(config, schemaName, { applicationRole: role });
      await DBOS.launch();
