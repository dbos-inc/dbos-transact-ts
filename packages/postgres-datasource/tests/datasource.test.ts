...
async function raceFunction() {
  ...
  await dataSource.client`INSERT INTO race_side_effects(tag) VALUES (${`run-${raceState.callCount}`})`;
...
const sysConfig = { ...config, database: 'pg_ds_test_dbos_sys' };
...
async function staleFunction() {
  await dataSource.client`INSERT INTO race_side_effects(tag) VALUES ('stale')`;
