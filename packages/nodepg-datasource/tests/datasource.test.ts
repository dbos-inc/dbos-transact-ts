...
async function raceFunction() {
  ...
  await NodePostgresDataSource.client.query('INSERT INTO race_side_effects(tag) VALUES ($1)', [
    `run-${raceState.callCount}`,
  ]);
...
describe('NodePostgresDataSource migrations', () => {
  const userDB = new Pool(config);
  ...
  test('verify-only initialization requires the schema to be migrated out of band', async () => {
    ...
    try {
      new NodePostgresDataSource(`verify-only-${suffix}`, config, schemaName, { runMigrations: false });
      DBOS.setConfig({ name: 'node-pg-ds-test' });
      ...
      await NodePostgresDataSource.initializeDBOSSchema(config, schemaName, { applicationRole: role });
      await DBOS.launch();
