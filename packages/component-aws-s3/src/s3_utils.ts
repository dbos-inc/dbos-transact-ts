...
export class DBOS_S3 extends ConfiguredInstance {
    ...
    constructor(name: string, readonly config: DBOSS3Config) {super(name);}
    ...
    @DBOS.step()
    async presignedGetURL(key: string, @ArgOptional expirationSecs: number = 3600, @ArgOptional options: S3GetResponseOptions = {})
    {
    ...
    @DBOS.step()
    async createPresignedPost(key: string, expirationSecs: number = 1200,
        @ArgOptional contentOptions?: {
            contentType?: string,
            contentLengthMin?: number,
            contentLengthMax?: number,
    ...
    @DBOS.workflow()
    async getFileReadURL(fileDetails: FileRecord, @ArgOptional expirationSec = 3600, @ArgOptional options: S3GetResponseOptions = {}) : Promise<string>
    {
    ...
    @DBOS.workflow()
    async writeFileViaURL(fileDetails: FileRecord,
