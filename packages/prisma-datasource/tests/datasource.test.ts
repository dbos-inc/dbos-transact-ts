...
async function raceFunction() {
  ...
  await dataSource.client.$executeRaw`INSERT INTO race_side_effects(tag) VALUES (${`run-${raceState.callCount}`})`;
...
const sysConfig = { ...config, database: 'prisma_ds_test_dbos_sys' };
...
async function staleFunction() {
  await dataSource.client.$executeRaw`INSERT INTO race_side_effects(tag) VALUES ('stale')`;
...
describe('PrismaDataSource migrations', () => {
  const userDB = new Pool(config);
  ...
  test('verify-only initialization requires the schema to be migrated out of band', async () => {
    ...
    try {
      new PrismaDataSource(`verify-only-${suffix}`, prisma, schemaName, { runMigrations: false });
      DBOS.setConfig({ name: 'prisma-ds-test' });
      ...
      await PrismaDataSource.initializeDBOSSchema(prisma, schemaName, { applicationRole: role });
      await DBOS.launch();
