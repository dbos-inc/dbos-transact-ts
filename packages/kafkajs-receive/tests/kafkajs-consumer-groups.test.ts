...
/**
 * Trips once both ordered groups hold a workflow in flight at the same instant. The group ID is
 * part of the partition key, so they occupy different keys and can overlap; were it dropped, they
 * would share one key on a queue that runs a single workflow per key and could never overlap.
 * Each arrival lingers, so the other group has a wide window to meet it rather than a race.
 */
class Rendezvous {
  readonly #inFlight = new Set<string>();
  #resolve!: () => void;
  tripped = false;
  readonly #both: Promise<void>;
  constructor(private readonly need: number) {
    this.#both = new Promise<void>((r) => (this.#resolve = r));
  }
  async meet(group: string, lingerMS: number) {
    this.#inFlight.add(group);
    if (this.#inFlight.size >= this.need) {
      this.tripped = true;
      this.#resolve();
    } else {
      // Bounded: if the groups cannot overlap, nobody is coming, so don't hang the suite.
      await Promise.race([this.#both, new Promise((r) => setTimeout(r, lingerMS))]);
    }
...
suite('kafkajs-receive-consumer-groups', async () => {
  ...
  await test(
    ...
    async () => {
      ...
      // The load-bearing assertion: both groups were in flight at once, so neither is blocking the
      // other. Order alone would still hold if they shared a key and ran strictly one at a time.
      assert.ok(
        orderedRendezvous.tripped,
        'the ordered groups never overlapped, so they are serializing against each other',
      );
