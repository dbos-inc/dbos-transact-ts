...
class DBOSCP implements ConfigProvider {
  ...
  getConfig<T>(key: string, defaultValue?: T) {
    return DBOS.getConfig(key, defaultValue);
  }
}
...
export function loadAWSConfigByName(ctx: ConfigProvider, cfgname: string): AWSServiceConfig {
  ...
  if (typeof cfgstrs.aws_access_key_id !== 'string') {
    throw new DBOSError.DBOSConfigKeyTypeError(
      `${cfgname}.aws_access_key_id`,
      'string',
      typeof cfgstrs.aws_access_key_id,
    );
  }
  ...
  if (typeof cfgstrs.aws_secret_access_key !== 'string') {
    throw new DBOSError.DBOSConfigKeyTypeError(
      `${cfgname}.aws_secret_access_key`,
      'string',
      typeof cfgstrs.aws_secret_access_key,
    );
  }
