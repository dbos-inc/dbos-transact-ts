...
// The broker-side setup uses kafkajs; only the receiver under test is the Confluent one.
const kafkaConfig: KafkaConfig = {
  clientId: 'dbos-conf-kafka-failure-test',
...
// what lets the test drive the failure paths by reassigning them.
...
const outageTopic = `dbos-conf-outage-${suffix}`;
const poisonTopic = `dbos-conf-poison-${suffix}`;
...
async function outageWorkflow(_topic: string, _partition: number, message: ConfluentKafkaJS.Message) {
  await Promise.resolve();
  outageRuns.push(Number(message.value!.toString()));
  if (outageRuns.length === NUM_MESSAGES) resolveOutage();
}

async function poisonWorkflow(_topic: string, _partition: number, message: ConfluentKafkaJS.Message) {
  await Promise.resolve();
  poisonRuns.push(Number(message.value!.toString()));
  if (poisonRuns.length === NUM_MESSAGES - 1) resolvePoison();
}

kafkaReceiver.registerConsumer(DBOS.registerWorkflow(outageWorkflow, { name: 'confOutageWorkflow' }), outageTopic, {
  name: 'confOutageWorkflow',
  config: { 'group.id': `dbos-conf-outage-grp-${suffix}` },
});
kafkaReceiver.registerConsumer(DBOS.registerWorkflow(poisonWorkflow, { name: 'confPoisonWorkflow' }), poisonTopic, {
  name: 'confPoisonWorkflow',
  config: { 'group.id': `dbos-conf-poison-grp-${suffix}` },
});
...
suite('confluent-kafka-receive-failure-paths', async () => {
  ...
  before(
    async () => {
      ...
      // Simulate a database outage for the first three enqueue attempts, whose 1s + 2s + 4s backoff
      // costs the consumer ~7s. Scope it to the outage topic, which the workflow ID carries: the
      // patch is global, and the poison consumer enqueues through here too.
      ...
      (eventreceiver as { prepareEnqueuedWorkflow: typeof originalPrepare }).prepareEnqueuedWorkflow = async (
        ...
        const message = (args as unknown as [string, number, ConfluentKafkaJS.Message])[2];
        if (options.queueName.includes('confluent') && Number(message.value!.toString()) === POISON_VALUE) {
          const topic = (args as unknown as [string, number, ConfluentKafkaJS.Message])[0];
      ...
      try {
        ...
        await dropDB(client, 'conf_kafka_failure_test_dbos_sys', true);
      } finally {
        await client.end();
      }

      DBOS.setConfig({ name: 'conf-kafka-failure-test' });
      await DBOS.launch();
    },
    { timeout: 60000 },
  );
  ...
  await test(
    ...
    async () => {
      ...
      // The idempotent insert held: the retried batch created each row once rather than duplicating it.
