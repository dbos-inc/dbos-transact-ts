...
export class SystemDatabase {
  ...
  private async recordOperationResultInternal(
    ...
    try {
      let recordedStartTimeEpochMs = startTimeEpochMs;
      if (this.isSQLiteSystemDatabase && functionName !== DBOS_FUNCNAME_SLEEP) {
        const { rows } = await client.query<{ created_at: number | string }>(
          `SELECT created_at FROM "${this.schemaName}".workflow_status WHERE workflow_uuid = $1`,
          [workflowID],
        );
        if (rows[0]) {
          // SQLite timestamps have millisecond precision. Keep operation time
          // strictly after the parent workflow's creation time.
          recordedStartTimeEpochMs = Math.max(startTimeEpochMs, Number(rows[0].created_at) + 1);
        }
      }
      const recordedEndTimeEpochMs =
        this.isSQLiteSystemDatabase && functionName !== DBOS_FUNCNAME_SLEEP
          ? Math.max(endTimeEpochMs, recordedStartTimeEpochMs + 1)
          : endTimeEpochMs;
