...
export class PostgresSystemDatabase implements SystemDatabase {
  ...
  async send(workflowUUID: string, functionID: number, destinationUUID: string, message: unknown, topic?: string): Promise<void> {
  ...
  async setEvent(workflowUUID: string, functionID: number, key: string, message: unknown): Promise<void> {
    const client: PoolClient = await this.pool.connect();
