...
export function registerFunction<T extends any[], R>(fn: OperonFunction<T, R>): RegisteredFunction<T, R> {
  return async function (ctxt: WorkflowContext, ...args: T): Promise<R> {
    ...
    async function checkExecution() : Promise<R | null> {
      const { rows } = await client.query("SELECT output FROM operon__FunctionOutputs WHERE workflow_id=$1 AND function_id=$2", [ctxt.workflowID, fCtxt.functionID]);
      if (rows.length === 0) {
        return null;
      } else {
    ...
    async function recordExecution(output: R) {
      await client.query("INSERT INTO operon__FunctionOutputs VALUES ($1, $2, $3)", [ctxt.workflowID, fCtxt.functionID, JSON.stringify(output)]);
    }
