...
export async function readConfigFile(dirPath: string | undefined): Promise<ConfigFile> {
  dirPath ??= process.cwd();
  const dbosConfigPath = path.join(dirPath, 'dbos-config.yaml');
  const configContent = await readFile(dbosConfigPath);

  const config: ConfigFile = configContent ? YAML.parse(substituteEnvVars(configContent)) : {};
  if (!config.name) {
    const packageJsonPath = path.join(dirPath, 'package.json');
    const packageContent = await readFile(packageJsonPath);
    config.name = packageContent ? JSON.parse(packageContent).name : undefined;
  }
...
//     run_admin_server: config.runAdminServer === undefined ? true : config.runAdminServer,
//     entrypoints: [],
//     start: [],
//     setup: [],
//   };
