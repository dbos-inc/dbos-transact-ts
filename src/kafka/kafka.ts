...
export function KafkaConsume(_topics: unknown, _consumerConfig?: unknown, _queueName?: unknown): never {
  throw new Error(
    '@KafkaConsume decorator has moved to the `@dbos-inc/dbos-kafkajs` package, please install and change the imports',
  );
}
...
export function Kafka(_kafkaConfig: unknown): never {
  throw new Error(
    '@Kafka decorator has moved to the `@dbos-inc/dbos-kafkajs` package, please install and change the imports',
  );
}
