...
describe('testing operon', () => {
  ...
  test('should abort function properly', async() => {
    ...
    const testFunctioRead = registerFunction(async (functionCtxt: FunctionContext, id: number) => {
    ...
    const testWorkflow = registerWorkflow(async (workflowCtxt: WorkflowContext, name: string) => {
      ...
      const checkResult: number = await testFunctioRead(workflowCtxt, funcResult);
      return checkResult;
    });
