...
class OperonTestClass {
  ...
  static async testKvFunctionRead (txnCtxt: TestTransactionContext, id: number) {
    const { rows } = await txnCtxt.client.query<TestKvTable>(
      `SELECT id FROM ${testTableName} WHERE id=$1`, [id]
    );
    if (rows.length > 0) {
      return Number(rows[0].id);
    } else {
  ...
  static async testOaooFunction (txnCtxt: TestTransactionContext, name: string) {
    const { rows } = await txnCtxt.client.query<TestKvTable>(
      `INSERT INTO ${testTableName}(value) VALUES ($1) RETURNING id`, [name]
    );
    return Number(rows[0].id);
  }
...
describe("operon-tests", () => {
  ...
  test("return-void", async () => {
    ...
    await expect(
      operon.transaction(OperonTestClass.testVoidFunction, { workflowUUID: workflowUUID })
    ).resolves.toBeFalsy();
    await expect(
      operon.transaction(OperonTestClass.testVoidFunction, { workflowUUID: workflowUUID })
    ).resolves.toBeFalsy();
  ...
  test("tight-loop", async () => {
    for (let i = 0; i < 100; i++) {
      await expect(
        operon.workflow(OperonTestClass.testNameWorkflow, {}, username).then(x => x.getResult())
  ...
  test("abort-function", async () => {
    for (let i = 0; i < 10; i++) {
      await expect(
        operon.workflow(OperonTestClass.testFailWorkflow, {}, username).then(x => x.getResult())
      ).resolves.toBe(i + 1);
    }

    // Should not appear in the database.
    await expect(
      operon.workflow(OperonTestClass.testFailWorkflow, {}, "fail").then(x => x.getResult())
  ...
  test("oaoo-simple", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      workflowResult = await operon
        .workflow(OperonTestClass.testOaooWorkflow, { workflowUUID: workflowUUID }, username)
        .then(x => x.getResult());
  ...
  test("simple-communicator", async () => {
    ...
    let result: number = await operon
      .workflow(OperonTestClass.testCommWorkflow, { workflowUUID: workflowUUID })
      .then(x => x.getResult());
  ...
  test("simple-workflow-notifications", async () => {
    ...
    const handle = await operon
      .workflow(OperonTestClass.receiveWorkflow, { workflowUUID: workflowUUID });
  ...
  test("readonly-recording", async () => {
    ...
    await expect(
      operon
        .workflow(OperonTestClass.testRecordingWorkflow, { workflowUUID: workflowUUID }, 123, "test")
        .then(x => x.getResult())
  ...
  test("retrieve-workflowstatus", async () => {
    ...
    const workflowHandle = await operon.workflow(
      OperonTestClass.testStatusWorkflow,
    ...
    ).resolves.toMatchObject({ status: StatusString.PENDING, workflowName: OperonTestClass.testStatusWorkflow.name });
