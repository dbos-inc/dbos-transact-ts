import { Operon, OperonConfig, TransactionContext, CommunicatorContext, WorkflowContext, StatusString } from 'src/';
...
describe('foundationdb-operon', () => {
  ...
  test('fdb-operon', async () => {
    ...
    const uuid = uuidv1();
    await expect(operon.transaction(testFunction, {workflowUUID: uuid})).resolves.toBe(5);
  ...
  test('fdb-error-recording',async () => {
    ...
    operon.registerTransaction(testFunction);
    const uuid = uuidv1();
    await expect(operon.transaction(testFunction, {workflowUUID: uuid})).rejects.toThrow("fail");
    await operon.flushWorkflowOutputBuffer();
    await expect(operon.transaction(testFunction, {workflowUUID: uuid})).rejects.toThrow("fail");
    expect(counter).toBe(1);
  });
  ...
  test('fdb-communicator', async() => {
    ...
    await expect(operon.workflow(testWorkflow, {workflowUUID: workflowUUID}).getResult()).resolves.toBe(0);
  ...
  test('fdb-communicator-error',async () => {
    ...
    await expect(operon.workflow(testWorkflow, {}).getResult()).resolves.toBe("success");

    const workflowUUID: string = uuidv1();
    await expect(operon.workflow(testWorkflow, {workflowUUID: workflowUUID}).getResult()).resolves.toBe("Communicator reached maximum retries.");
    await expect(operon.workflow(testWorkflow, {workflowUUID: workflowUUID}).getResult()).resolves.toBe("Communicator reached maximum retries.");
  });
  ...
  test('fdb-workflow-status',async () => {
    ...
    const uuid = uuidv1();
    await expect(operon.workflow(testWorkflow, {workflowUUID: uuid}).getResult()).resolves.toBe(3);
    
    const retrievedHandle = operon.retrieveWorkflow(uuid);
    await expect(retrievedHandle.getResult()).resolves.toBe(3);
    await expect(retrievedHandle.getStatus()).resolves.toMatchObject({status: StatusString.SUCCESS});
    expect(counter).toBe(1);
  });
  ...
  test('fdb-notifications', async () => {
    ...
    const success = await operon.send({}, "test-topic", "test-key", "test-message");
  ...
  test('fdb-duplicate-notifications',async () => {
    ...
    await expect(operon.retrieveWorkflow(sendUUID).getResult()).resolves.toBe(true);
