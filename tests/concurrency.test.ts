...
describe("concurrency-tests", () => {
  ...
  test("duplicate-transaction", async () => {
    ...
    cnt = 0;
    const testFunction2 = async (txnCtxt: TransactionContext, id: number) => {
      await sleep(10);
      cnt += 1;
      return id;
    };
    operon.registerTransaction(testFunction2, { readOnly: true });
    const readUUID = uuidv1();
    results = await Promise.allSettled([
      operon.transaction(testFunction2, { workflowUUID: readUUID }, 12),
      operon.transaction(testFunction2, { workflowUUID: readUUID }, 12),
    ]);
